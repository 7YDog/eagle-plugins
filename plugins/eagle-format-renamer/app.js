(function () {
  ...
  const AUTO_REFRESH_MS = 450;
  const SELECTION_READ_TIMEOUT_MS = 1500;
  const MAX_LOGS = 200;
  const DEFAULT_HISTORY = [
    { pattern: "%F-周辉发布%N", start: 1, step: 1, padding: 0, dateSource: "today", dateFormat: "M月D日", sanitize: true, uses: 0 },
  ...
  function bindEvents() {
    ...
    window.addEventListener("pageshow", () => checkSelectionChange("pageshow"));
  ...
  function bootEagle() {
    ...
    const registerLifecycle = () => {
      if (!window.eagle || lifecycleRegistered) {
        return;
      }

      let registered = false;
      if (typeof window.eagle.onPluginCreate === "function") {
        window.eagle.onPluginCreate(start);
        registered = true;
      }
      if (typeof window.eagle.onPluginShow === "function") {
        window.eagle.onPluginShow(() => {
          if (state.running) {
            return;
          }
          refreshSelectedItems({ silent: true, reason: "plugin-show" });
        });
        registered = true;
      }
      if (typeof window.eagle.onLibraryChanged === "function") {
        window.eagle.onLibraryChanged(() => {
          state.lastSelectionKey = "";
          if (!state.running) {
            refreshSelectedItems({ silent: true, reason: "library-change" });
          }
        });
        registered = true;
      }
    ...
    const tryStart = () => {
      ...
      logEvent("api_missing", { reason: "startup-timeout" });
    };
  ...
  async function refreshSelectedItems(options = {}) {
    ...
    try {
      ...
      const selected = options.selected !== undefined ? options.selected : await readCurrentSelection();
      state.items = normalizeItems(selected);
      ...
      renderPreview();
      logEvent("selection_refreshed", {
        reason: options.reason || "manual",
        count,
        reader: state.selectionReader || "provided-selection"
      });
      return true;
    } catch (error) {
      if (!silent) {
  ...
  async function checkSelectionChange(reason) {
    if (state.running || state.refreshInFlight || state.selectionPollInFlight || !hasSelectionReader()) {
      return;
    }

    state.selectionPollInFlight = true;
    try {
      const nextItems = await readCurrentSelection();
      const nextKey = selectionKey(nextItems);
      if (nextKey !== state.lastSelectionKey) {
        const refreshed = await refreshSelectedItems({ silent: true, reason: "selection-change", selected: nextItems });
        if (refreshed) {
  ...
  function getSelectionReaders() {
    const itemApi = window.eagle && window.eagle.item;
    if (!itemApi) {
      return [];
    }
    ...
    if (typeof itemApi.getSelected === "function") {
      readers.push({
        name: "item.getSelected()",
        read: () => itemApi.getSelected()
      });
    }
    if (typeof itemApi.get === "function") {
      readers.push({
        name: "item.get({ isSelected: true })",
        read: () => itemApi.get({ isSelected: true })
      });
    }
    return readers;
  }

  function hasSelectionReader() {
    return getSelectionReaders().length > 0;
  }

  async function readCurrentSelection() {
    const readers = getSelectionReaders();
    if (readers.length === 0) {
      throw new Error("当前 Eagle API 不支持读取选中项");
    }
    ...
    for (const reader of readers) {
      try {
        const items = normalizeItems(await readWithTimeout(reader));
        successful = true;
        state.selectionReader = reader.name;
        if (items.length > 0) {
          return items;
        }
        emptyResult = items;
        ...
        lastError = error;
        logEvent("selection_reader_failed", { reader: reader.name, error: readableError(error) });
      }
    }
    ...
    throw lastError || new Error("读取 Eagle 选中项失败");
  }
    ...
    try {
      return await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(() => reject(new Error(`${reader.name} 读取超时`)), SELECTION_READ_TIMEOUT_MS);
        })
      ]);
    } finally {
      if (timer) {
  ...
  function normalizeItems(value) {
    ...
    if (Array.isArray(value.data)) {
      return normalizeItems(value.data);
    }
    if (value.data && typeof value.data === "object") {
      const nestedData = normalizeItems(value.data);
      if (nestedData.length > 0) {
        return nestedData;
      }
    }
    if (Array.isArray(value.items)) {
      return normalizeItems(value.items);
    }
  ...
  function selectionKey(items) {
    return items
      .map((item) => String(item.id || item.filePath || item.fileURL || item.path || item.name || ""))
      .filter(Boolean)
      .sort()
      .join("|");
  }
  ...
  function applySettings(settings, options = {}) {
    dom.patternInput.value = settings.pattern || "%F-周辉发布%N";
  ...
  function restoreSettings() {
    try {
      ...
      applySettings({
        pattern: saved.pattern || "%F-周辉发布%N",
        start: saved.start ?? 1,
        step: saved.step ?? 1,
