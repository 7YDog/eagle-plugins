(function () {
  ...
  async function ensureFfmpeg() {
    ...
    // 1. 尝试 Eagle 官方 extraModule API（与首帧封面插件一致）
    if (eagle.extraModule && eagle.extraModule.ffmpeg) {
      try {
        ...
        if (isInstalled) {
          ...
          const p = paths.ffmpeg || paths.ffmpegPath || paths.bin || paths.path;
          if (p && fs.existsSync(p)) {
            writeLog("info", { message: "使用 Eagle extraModule FFmpeg 路径", path: p });
            return p;
          }
        }
      } catch (e) {
        writeLog("warn", `eagle.extraModule.ffmpeg 检测异常: ${serialize(e)}`);
      }
    }

    // 2. 平台感知的 Eagle Plugins 内 FFmpeg 扩展路径
    const home = os.homedir();
    const platform = process.platform;
    const pluginCandidates = [];

    if (platform === "darwin") {
      pluginCandidates.push(path.join(home, "Library/Application Support/Eagle/Plugins/ffmpeg-mac-arm64/ffmpeg"));
    } else if (platform === "win32") {
      pluginCandidates.push(path.join(home, "AppData/Roaming/Eagle/Plugins/ffmpeg-win-x64/ffmpeg.exe"));
    } else {
      pluginCandidates.push(
        path.join(home, "Library/Application Support/Eagle/Plugins/ffmpeg-mac-arm64/ffmpeg"),
        path.join(home, "AppData/Roaming/Eagle/Plugins/ffmpeg-win-x64/ffmpeg.exe")
      );
    }

    for (const cand of pluginCandidates) {
      if (fs.existsSync(cand)) {
        writeLog("info", { message: "使用 Eagle Plugins FFmpeg 扩展", path: cand });
        return cand;
      }
    }

    // 3. 系统/Homebrew 仅作次级回退（不再作为主路径）
    const systemCandidates = platform === "win32"
      ? []
      : ["/opt/homebrew/bin/ffmpeg", "/usr/local/bin/ffmpeg", "/usr/bin/ffmpeg"];
    for (const cand of systemCandidates) {
      if (fs.existsSync(cand)) {
        writeLog("info", { message: "使用系统 FFmpeg", path: cand });
        return cand;
      }
    }

    // 4. 最后兜底：PATH 中的 ffmpeg，并提示安装 Eagle FFmpeg 扩展
    writeLog("warn", "未找到 Eagle FFmpeg 扩展；请在 Eagle 中安装 FFmpeg 扩展。暂时回退到 PATH 中的 ffmpeg。");
    return "ffmpeg";
  }
