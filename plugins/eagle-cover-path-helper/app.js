(function () {
  ...
  function boot() {
    ...
    // 仅在用户点击插件 / 快捷键触发 onPluginRun 时复制。
    // 不再在 Eagle 启动时 fallback 执行，避免无选中就弹「请先选中素材」。
    if (!supportsRunEvent) {
      logEvent("no_plugin_run_api", {
        note: "当前 Eagle 无 onPluginRun；封面路径助手不会在启动时自动运行"
      });
    }
  ...
  async function executeCopy(reason) {
    ...
    try {
      ...
      if (items.length === 0) {
        // 启动类 reason 静默返回；仅用户主动运行时提示
        if (reason === "plugin-run") {
